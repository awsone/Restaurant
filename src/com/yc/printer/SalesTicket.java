...
public class SalesTicket  implements Printable{
	...
	public String toString() {
		return "SalesTicket [goods=" + goods + ", operatorName=" + operatorName + ", orderId=" + orderId
				...
				+ integral + ",deskNum="+deskNum+"]";
	}
	public SalesTicket(List<GoodsInfo> goods, String operatorName, String orderId, String totalGoodsNum,
			String totalPrice, String actualCollection, String giveChange, String cardNumber, String integral,String deskNum) {
		super();
		this.goods = goods;
	...
	public SalesTicket(List<GoodsInfo> goods, String operatorName, String orderId, String totalGoodsNum,
			String totalPrice, String actualCollection, String giveChange,String deskNum) {
		super();
		this.goods = goods;
		this.operatorName = operatorName;
		this.orderId = orderId;
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		g2.setFont(new Font("楷体",Font.CENTER_BASELINE,12));//设置标题打印字体     

		float heigth = font.getSize2D();//获取字体的高度  

		//设置小票的标题标题  
		g2.drawString("味道先生（消费单）",(float)x+30,(float)y+heigth); 

		float line = 3*heigth; //下一行开始打印的高度
		g2.setFont(new Font("宋体", Font.BOLD,14));//设置正文字体  
		heigth = font.getSize2D();// 字体高度  

		line+=2;
		//设置餐桌号
		g2.drawString("餐桌号："+deskNum,(float)x+40,(float)y+heigth+20); 
		...
		//设置操作员 
		g2.drawString("操作员:"+operatorName,(float)x+60,(float)y+line+20); 
		line+=heigth;

		//设置订单号  
		g2.drawString("订单号:"+orderId, (float)x+60,(float)y+line);  
		line+=heigth+10; 
		...
		if(goods!=null && goods.size()>0){
			for(GoodsInfo gdf:goods){
				...
				g2.drawString(gdf.getPrice(),(float)x+65, (float)y+line);  
		...
		g2.setFont(new Font("宋体",font.CENTER_BASELINE,8));
		g2.drawString("时间:"+sdf.format(new Date()),(float)x+15,(float)y+line+15);  
		line += heigth; 
		g2.drawString("钱票请当面点清，离开柜台恕不负责",(float)x+15,(float)y+line+15);  
		switch (pageIndex) {  
		case 0:  
